...
class DnCNN_BF(nn.Module):
	"""DnCNN as defined in https://arxiv.org/abs/1608.03981 
	   reference implementation: https://github.com/SaoYan/DnCNN-PyTorch"""
	def __init__(self, depth=20, n_channels=64, image_channels=1, out_channel = 1, bias=False, kernel_size=3):
		super(DnCNN_BF, self).__init__()
		kernel_size = 3
		padding = 1
		...
		self.last_layer = nn.Conv2d(in_channels=n_channels, out_channels=out_channel, kernel_size=kernel_size, padding=padding, bias=self.bias)
	...
	def forward(self, x):
		...
		return y[:,0,:,:].unsqueeze(1) - out[:,0,:,:].unsqueeze(1)
